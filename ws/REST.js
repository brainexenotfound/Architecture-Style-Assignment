...
REST_ROUTER.prototype.handleRoutes= function(router,connection) {
    ...
    router.get("/orders/:order_id",function(req,res){
        ...
        let token = req.headers.token;
        if (!token || !self.sessions[token]) {
            logger.logAction(username, "UNAUTHORIZED GET_ORDER_BY_ID", false);
            return res.status(401).json({ error: true, message: "Unauthorized" });
        }
    ...
    router.post("/orders",function(req,res){
        ...
        let token = req.headers.token;
        if (!token || !self.sessions[token]) {
            logger.logAction(username, "UNAUTHORIZED CREATE_ORDER", false);
            return res.status(401).json({ error: true, message: "Unauthorized" });
        }
    ...
    router.delete("/orders/:order_id",function(req,res){
        let token = req.headers.token;
        if (!token || !self.sessions[token]) {
            logger.logAction(username, "UNAUTHORIZED DELETE_ORDER", false);
            return res.status(401).json({ error: true, message: "Unauthorized" });
        }
